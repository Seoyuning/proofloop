...
/** 학생 질문이 아직 없을 때만 보여주는 '예시' 미리보기 (학생이 질문하면 실데이터로 교체) */
const SAMPLE_CLUSTERS = [
  { q: "이차함수 표준형에서 꼭짓점이 왜 (p, q)예요?", unit: "3단원 이차함수", freq: 12, misconception: "p의 부호를 그대로 옮기는 실수" },
  { q: "판별식 D<0이면 왜 실근이 없나요?", unit: "2단원 이차방정식", freq: 8, misconception: "판별식 부호와 근의 개수 관계 혼동" },
...
interface Member {
  ...
  email: string;
  questionCount: number;
  weakConcepts: MemberWeak[];
}

export default function TeacherAnalysisPage() {
  const router = useRouter();
  const { user, isLoading } = useAuth();
  const { currentBot, activeClassId } = useStudio();
  ...
  // 활성 반의 실제 학생/약점 데이터
  useEffect(() => {
    if (!activeClassId) { setMembers([]); return; }
    setMembersLoading(true);
    fetch(`/api/classes/${activeClassId}/members`)
      .then((r) => r.json())
  ...
  // 반 전체 클러스터: 학생들의 약점을 (단원+오개념)으로 묶어 집계
  const clusterMap = new Map<string, { sectionTitle: string; misconception: string; freq: number; students: Set<string>; lastQuestion: string }>();
  for (const m of members) {
    for (const wc of m.weakConcepts) {
      const key = `${wc.sectionTitle}||${wc.misconception}`;
      const e = clusterMap.get(key) ?? { sectionTitle: wc.sectionTitle, misconception: wc.misconception, freq: 0, students: new Set<string>(), lastQuestion: wc.lastQuestion };
      e.freq += wc.count;
      e.students.add(m.studentId);
      e.lastQuestion = wc.lastQuestion;
      clusterMap.set(key, e);
    }
  }
  const realClusters = Array.from(clusterMap.values()).sort((a, b) => b.freq - a.freq);
  const hasStudents = members.length > 0;
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <div className="group relative overflow-hidden rounded-[22px] border border-line bg-white/90 p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
              ...
              <p className="relative mt-3 text-2xl font-bold tabular-nums tracking-tight text-navy">{dashboard?.totalQuestions ?? 0}<span className="ml-0.5 text-sm font-semibold text-muted">회</span></p>
            </div>
            <div className="group relative overflow-hidden rounded-[22px] border border-line bg-white/90 p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
              ...
              <p className="relative mt-3 text-2xl font-bold tabular-nums tracking-tight text-navy">{dashboard?.totalStudents ?? members.length}<span className="ml-0.5 text-sm font-semibold text-muted">명</span></p>
            </div>
            <div className="group relative overflow-hidden rounded-[22px] border border-line bg-white/90 p-5 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
              ...
              <p className="relative mt-3 text-sm font-bold leading-5 text-navy">{dashboard?.topMisconception ?? realClusters[0]?.misconception ?? "없음"}</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <section className="app-panel rounded-[28px] p-5 sm:p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <SectionHeader
                ...
                title="통합 질문 클러스터"
                copy={hasStudents
                  ? "학생들의 질문을 단원·오개념으로 묶어 반 공통 약점을 보여줍니다."
                  : "아직 학생 질문이 없어 예시를 보여줍니다. 학생이 질문하면 실데이터로 교체됩니다."}
              />
              <div className="flex items-center gap-2">
                {!hasStudents && (
                  <span className="whitespace-nowrap rounded-full bg-orange/10 px-3 py-1 text-xs font-semibold text-orange">예시 미리보기</span>
                )}
                <div className="rounded-[20px] border border-line bg-white px-4 py-3 text-sm text-muted">
                  클러스터 {hasStudents ? realClusters.length : 0}개
                </div>
              </div>
            </div>
              ...
              {!hasStudents ? (
                <>
                  {SAMPLE_CLUSTERS.map((c) => (
                    <div key={c.q} className="rounded-[22px] border border-dashed border-line bg-white/45 p-4">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <span className="whitespace-nowrap rounded-full bg-navy/40 px-3 py-1 text-xs font-medium text-white">{c.freq}회</span>
                      </div>
                      <div className="mt-4">
                  ...
                  <p className="pt-1 text-center text-xs text-muted/60">
                    {membersLoading ? "불러오는 중…" : "위는 예시입니다. 학생이 참여해 질문하면 이 자리에 실데이터가 쌓입니다."}
                  </p>
                </>
              ) : realClusters.length === 0 ? (
                <div className="rounded-[22px] border border-line bg-white/72 p-6 text-center">
                  <p className="text-sm text-muted">참여 학생은 있지만 아직 오개념으로 묶일 질문이 부족합니다.</p>
                </div>
              ) : (
                realClusters.map((c) => (
                  <div key={`${c.sectionTitle}-${c.misconception}`} className="rounded-[22px] border border-line bg-white/72 p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <p className="text-base font-semibold text-navy">{c.lastQuestion}</p>
                        <p className="mt-1 text-sm text-muted">{c.sectionTitle}</p>
                      </div>
                      <span className="whitespace-nowrap rounded-full bg-navy px-3 py-1 text-xs font-medium text-white">{c.freq}회</span>
                    </div>
                    <div className="mt-4 grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
                      <InfoBlock label="오개념 태그" value={c.misconception} />
                      <InfoBlock label="공통 학생" value={`${c.students.size}명에게 공통`} />
                    </div>
                  </div>
                ))
              )}
              ...
              <div className="relative">
                ...
                {showUploadMenu && (
                  <div className="absolute right-0 top-full z-20 mt-2 w-48 rounded-[18px] border border-line bg-white p-2 shadow-xl">
                    <button type="button" className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-navy transition-colors hover:bg-surface-strong" onClick={() => cameraInputRef.current?.click()}>
                      <span className="text-base">📷</span> 사진 촬영
                    </button>
                    <button type="button" className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-navy transition-colors hover:bg-surface-strong" onClick={() => { fileInputRef.current?.setAttribute("accept", "image/*"); fileInputRef.current?.click(); }}>
                      <span className="text-base">🖼️</span> 사진 선택
                    </button>
                    <button type="button" className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm text-navy transition-colors hover:bg-surface-strong" onClick={() => { fileInputRef.current?.setAttribute("accept", ".pdf,.doc,.docx,.txt,.hwp"); fileInputRef.current?.click(); }}>
                      <span className="text-base">📄</span> 파일 올리기
                    </button>
                    <button type="button" className="mt-1 w-full rounded-xl px-3 py-2 text-left text-xs text-muted transition-colors hover:bg-surface-strong" onClick={() => setShowUploadMenu(false)}>
                      닫기
                    </button>
                  </div>
                )}
        ...
        <div className="space-y-4">
          <section className="app-panel rounded-[28px] p-5 sm:p-6">
            <SectionHeader
              kicker="학생별 약점"
              title="개인별 약점 트래킹"
              copy="이 반 학생 개개인이 어떤 개념에서 반복적으로 어려움을 겪는지 확인합니다."
            />
            ...
            <div className="app-scroll mt-6 max-h-[700px] space-y-4 overflow-y-auto pr-1">
              {membersLoading ? (
                ...
                <p className="text-sm text-muted">아직 참여한 학생이 없습니다. 반 관리에서 초대 코드를 학생에게 공유하세요.</p>
              ) : (
                members.map((m) => (
                  <div key={m.studentId} className="rounded-[22px] border border-line bg-white/72 p-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-base font-semibold text-navy">{m.name || "이름 없음"}</p>
                        <p className="text-sm text-muted">약점 {m.weakConcepts.length}개 · 질문 {m.questionCount}건</p>
                      </div>
                    </div>

                    {m.weakConcepts.length === 0 ? (
                      <p className="mt-4 rounded-[18px] border border-line bg-surface-strong p-3 text-sm text-muted">
                        {m.questionCount > 0 ? "질문은 있으나 아직 오개념으로 분류된 항목이 없습니다." : "아직 질문 기록이 없습니다."}
                      </p>
                    ) : (
                        ...
                        {m.weakConcepts.map((wc, i) => (
                          <div key={i} className="rounded-[18px] border border-line bg-surface-strong p-3">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                              ...
                              <span className="whitespace-nowrap rounded-full bg-orange/10 px-3 py-1 text-xs font-semibold text-orange">{wc.count}회 반복</span>
                            </div>
                            <p className="mt-2 text-sm leading-6 text-muted">최근 질문: &ldquo;{wc.lastQuestion}&rdquo;</p>
                          </div>
                        ))}
                      </div>
                    )}
